...
import com.youwonn_invest.ui.view.MNViderPlayer;
    ...
    private void initPlayer() {
        ...
        mnViderPlayer.setOnNetChangeListener(new MNViderPlayer.OnNetChangeListener() {
            ...
            public void onMobile(MediaPlayer mediaPlayer) {
                Toast.makeText(LiveRoomActivity.this, "请注意,当前网络状态切换为3G/4G网络", Toast.LENGTH_LONG).show();
            }

            @Override
            public void onNoAvailable(MediaPlayer mediaPlayer) {
                Toast.makeText(LiveRoomActivity.this, "当前网络不可用,检查网络设置", Toast.LENGTH_LONG).show();
            }
        });
    }
...
//            ((InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(LiveRoomActivity.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
